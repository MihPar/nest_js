...
export class RegistrationConfirmation {
	constructor(
		public inputDateRegConfirm: InputDateReqConfirmClass
	) {}
}

@CommandHandler(RegistrationConfirmation)
export class RegistrationConfirmationCase implements ICommandHandler<RegistrationConfirmation> {
	constructor(
		protected readonly usersQueryRepository: UsersQueryRepository,
		protected readonly usersRepository: UsersRepository
	) {}
	async execute(command: RegistrationConfirmation) {
		const user = await this.usersQueryRepository.findUserByConfirmation(command.inputDateRegConfirm.code);
